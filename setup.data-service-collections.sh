...
echo "Setting up Data Service Collections environment..."
...
echo "Adding database privileges..."
  ...
  GRANT ALL PRIVILEGES ON \`data-service-collections\`.* TO 'sail'@'%';
  FLUSH PRIVILEGES;
...
find /var/www/data-service-collections \( -name ".git" -o -name "node_modules" -o -name "storage" \) -prune -o -type f -exec chown sail:sail {} +
find /var/www/data-service-collections \( -name ".git" -o -name "node_modules" -o -name "storage" \) -prune -o -type d -exec chown sail:sail {} +
chown -R sail:sail /run/php

# Fix specific Laravel directories that need write permissions
chown -R sail:sail /var/www/data-service-collections/storage || true
chown -R sail:sail /var/www/data-service-collections/bootstrap/cache || true

# Change to the Laravel application directory
cd /var/www/data-service-collections
...
if [ -d "$DUMPS_DIR" ]; then
    # Find the latest data-service-collections dump file
    LATEST_DUMP=$(find "$DUMPS_DIR" -name "data-service-collections*.sql" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
    ...
    if [ -n "$LATEST_DUMP" ] && [ -f "$LATEST_DUMP" ]; then
        ...
        echo "No data-service-collections dump found"
    fi
else
    echo "Dumps directory not found"
fi

echo "Data Service Collections setup complete!"
