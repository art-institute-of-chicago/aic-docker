...
echo "Setting up Data Service Events environment..."
...
echo "Adding database privileges..."
  ...
  GRANT ALL PRIVILEGES ON \`data-service-events\`.* TO 'sail'@'%';
  FLUSH PRIVILEGES;
...
find /var/www/data-service-events \( -name ".git" -o -name "node_modules" -o -name "storage" \) -prune -o -type f -exec chown sail:sail {} +
find /var/www/data-service-events \( -name ".git" -o -name "node_modules" -o -name "storage" \) -prune -o -type d -exec chown sail:sail {} +
chown -R sail:sail /run/php

# Fix specific Laravel directories that need write permissions
chown -R sail:sail /var/www/data-service-events/storage || true
chown -R sail:sail /var/www/data-service-events/bootstrap/cache || true

# Change to the Laravel application directory
cd /var/www/data-service-events
...
if [ -d "$DUMPS_DIR" ]; then
    # Find the latest data-service-events dump file
    LATEST_DUMP=$(find "$DUMPS_DIR" -name "data-service-events*.sql" -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -1 | cut -d' ' -f2-)
    ...
    if [ -n "$LATEST_DUMP" ] && [ -f "$LATEST_DUMP" ]; then
        ...
        echo "No data-service-events dump found"
    fi
else
    echo "Dumps directory not found"
fi

echo "Data Service Events setup complete!"
